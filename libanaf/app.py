...
@app.callback()
def main(ctx: typer.Context, verbose: Annotated[bool, typer.Option(False, "--verbose", "-v", help="Enable verbose output")]) -> None:
...
def auth() -> None:
    ...
    client = LibANAF_AuthClient(config["auth"]["client_id"], config["auth"]["client_secret"], config["auth"]["auth_url"], config["auth"]["token_url"], config["auth"]["redirect_uri"])
